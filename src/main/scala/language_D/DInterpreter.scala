...
import libs.{Expr, emptyExpression}

import scala.language.postfixOps

class DInterpreter extends BachtInterpreter(new DStore)
{
    ...
    def process(expression: Expr): (Boolean, Expr) =
    {
        ...
        expression match
        {
            case primitiveExpression(primitive, token)                    =>
            {
                if ( exec_primitive(primitive, token) )
                {
                    println(primitive)
            ...
            case composedExpression(";", leftExpression, rightExpression) => process(leftExpression) match
            {
            ...
            process(x) match
            {
                case (false, delayExpression(time)) => process(y) match
                {
    ...
    def executeExpressions(agent: Expr): Boolean =
    {
        ...
        {
            encounteredFailure = process(currentExpression) match
            {
